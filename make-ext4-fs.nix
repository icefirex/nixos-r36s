# Fork of nixpkgs make-ext4-fs.nix with U-Boot-compatible ext4 features.
# Disables metadata_csum, 64bit, and orphan_file which U-Boot's ext4 driver
# cannot handle.
{
  pkgs,
  lib,
...
pkgs.stdenv.mkDerivation {
  ...
  buildCommand = ''
    ...
    # Disable ext4 features unsupported by U-Boot's ext4 driver
    faketime -f "1970-01-01 00:00:01" fakeroot mkfs.ext4 \
      -O ^metadata_csum,^64bit,^orphan_file \
      -L ${volumeLabel} -U ${uuid} -d ./rootImage $img
